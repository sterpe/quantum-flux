...
function register(callback) {
	...
	store.callback = function (payload) {
		try {
			...
			store.state = 'complete';
			//Re-throw in a lower priority turn.
...
function dispatch(payload) {
	...
	return this._startDispatching(payload);
}
...
function _startDispatching(payload) {
	...
	Q.allSettled(promises).then(function (results) {
		...
		onStoresUpdated.resolve(0);
	});
	this.push(payload);
	return {
		onStoresUpdated: onStoresUpdated.promise
	};
}
